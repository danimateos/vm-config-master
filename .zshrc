...
if [ "`id -u`" -eq 0 ]; then
    PS1="%{[31m%}%n%{[37m%}@%{[36m%}%m %{[37;1m%}%1~ %{[30;1m%}%# %{[0m%}"
else
    PS1="%{[33m%}%n%{[37m%}@%{[36m%}%m %{[37;1m%}%1~ %{[30;1m%}%# %{[0m%}"
fi
...
export PATH=$PATH:/usr/local/bin/popcorntime
...
fontsize(){
    newsize="$1"
    sed -si 's/pixelsize=[0-9]\+,/pixelsize='$newsize',/g' ~/.Xdefaults
}

# added by Anaconda3 2018.12 installer
# >>> conda init >>>
# !! Contents within this block are managed by 'conda init' !!
__conda_setup="$(CONDA_REPORT_ERRORS=false '/home/dsc/anaconda3/bin/conda' shell.bash hook 2> /dev/null)"
if [ $? -eq 0 ]; then
    \eval "$__conda_setup"
else
    if [ -f "/home/dsc/anaconda3/etc/profile.d/conda.sh" ]; then
        . "/home/dsc/anaconda3/etc/profile.d/conda.sh"
        CONDA_CHANGEPS1=false conda activate base
    else
        \export PATH="/home/dsc/anaconda3/bin:$PATH"
    fi
fi
unset __conda_setup
# <<< conda init <<<
